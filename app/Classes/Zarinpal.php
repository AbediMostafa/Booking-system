...
class Zarinpal
{
    const MERCHANT_ID = "47d9f4f7-1d4a-45a9-b1ca-43532547006b";

    public $email = '';
    public $mobile = '';
    public $ZarinGate = false;
    ...
    public function request($initialValues)
    {
        ...
        $node = ($this->SandBox == true) ? "sandbox" : "ir";
    ...
    {
        return isset($this->result["Status"]) && $this->result["Status"] == 100;
    }

    public function then($payment)
    {
        $payment->description = 'redirecting to payment page ...';
        $payment->save();
        // Success and redirect to pay
        $this->redirect($this->result["StartPay"]);

        return $this;
    }

    public function catch($payment)
    {
        $payment->status = 'failed';
        $payment->description = $this->result["Status"] . ' - ' . $this->result["Message"];
        $payment->save();
    }

    public function verify($Amount, $SandBox = false, $ZarinGate = false)
    {
        ...
        $data = array('MerchantID' => self::MERCHANT_ID, 'Authority' => $au, 'Amount' => $Amount);
        $jsonData = json_encode($data);
